...
def escala(tonica: str, tonalidade: str) -> dict[str, list[str]]:
    ...
    Parameters:
        tonica: Nota que será a tônica da escola
    ...
    Raises:
        ValueError: Caso uma nota tônica não seja válida.
    ...
    try:
        ...
        raise ValueError(f'Essa nota não existe, as que existem são: {NOTAS}')
    except KeyError:
        raise KeyError(
            f'Essa escala não existe ou não foi implementada. As escalas disponíveis são: {list(ESCALAS.keys())}'
        )
